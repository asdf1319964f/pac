...
const Utils = {
  ...
  async asyncPool(tasks, limit = CONSTANTS.CONCURRENCY_LIMIT) {
    ...
    async function runner() {
      while (idx < tasks.length) {
        ...
        if (!Utils.isFunc(t)) {
          res[cur] = { __error: new Error(`任务 ${cur} 不是函数`) };
          continue;
        }
        ...
        try {
          ...
          res[cur] = { __error: e || new Error("任务执行失败") };
        }
      }
    }
  ...
  calculateWeightedAverage(values, weightFactor = 0.9) {
    ...
    for (let i = 0; i < n; i++) {
      const w = Math.pow(weightFactor, n - i - 1);
      sum += values[i] * w;
      wsum += w;
    }
  ...
  calculateTrend(values) {
    ...
    const den = sw * sx2 - sx * sx;
    return den === 0 ? 0 : (sw * sxy - sx * sy) / den;
  },
  ...
  isValidDomain(d) {
    return typeof d === "string" && /^[a-zA-Z0-9.-]+$/.test(d) && !d.startsWith(".") && !d.endsWith(".") && !d.includes("..");
  },
  ...
  createServiceGroups(config, regionGroupNames, ruleProviders, rules) {
    ...
    const qualityOf = (name) => {
      try {
        ...
        const m = st.metrics || {};
        const metricScore = CentralManager.scoreComponents(m).metricScore;
...
class InvalidRequestError extends Error {
  constructor(m) { super(m); this.name = "InvalidRequestError"; }
}
...
class EventEmitter {
  ...
  on(ev, fn) {
    if (!ev || !Utils.isFunc(fn)) return;
    const arr = this.eventListeners.get(ev) || [];
    arr.push(fn);
    this.eventListeners.set(ev, arr);
  }
  ...
  off(ev, fn) {
    ...
    if (!arr.length) this.eventListeners.delete(ev);
  }
  
  emit(ev, ...args) {
    const arr = this.eventListeners.get(ev);
    if (!arr?.length) return;
    for (const fn of (arr.length > 1 ? [...arr] : arr)) {
      try {
        fn(...args);
      } catch (e) {
        Logger.error(`事件 ${ev} 处理失败:`, e.stack || e);
      }
    }
  }
  
  removeAllListeners(ev) {
    if (ev) this.eventListeners.delete(ev);
...
class LRUCache {
  ...
  _unlink(n) {
    if (!n || n === this.head || n === this.tail) return;
    const { prev, next } = n;
  ...
  _pushFront(n) {
    ...
    if (this.head.next) this.head.next.prev = n;
    this.head.next = n;
  }
  
  _evictTail() {
    const n = this.tail.prev;
    if (!n || n === this.head) return null;
    this._unlink(n);
    this.cache.delete(n.key);
    return n.key;
  }
  ...
  get(key) {
    ...
    if ((Utils.now() - e.timestamp) > e.ttl) {
      this._unlink(e);
...
async function __probeMirror(prefix, fetchFn, timeoutMs) {
  ...
  try {
    ...
    const tid = timeoutMs > 0 ? setTimeout(() => {
      try { c?.abort(); } catch {}
    }, timeoutMs) : null;
...
async function selectBestMirror(runtimeFetch) {
  ...
  __ghSelectLock = __ghSelectLock.then(async () => {
    const results = await Promise.all(
      GH_MIRRORS.map(m => 
        __probeMirror(m, runtimeFetch, CONSTANTS.GEO_INFO_TIMEOUT)
          .then(ok => ({ m, ok }))
          .catch(() => ({ m, ok: false }))
      )
    );
...
const Config = {
  ...
  services: [
    {
      id: "openai",
      rule: ["DOMAIN-SUFFIX,grazie.ai,国外AI", "DOMAIN-SUFFIX,grazie.aws.intellij.net,国外AI", "RULE-SET,ai,国外AI"],
      name: "国外AI",
      url: "https://chat.openai.com/cdn-cgi/trace",
      icon: ICON_VAL(ICONS.ChatGPT),
      ruleProvider: {name: "ai", url: URLS.rulesets.ai()}
    },
    {
      id: "youtube",
      rule: ["GEOSITE,youtube,YouTube"],
      name: "YouTube",
      url: "https://www.youtube.com/s/desktop/494dd881/img/favicon.ico",
      icon: ICON_VAL(ICONS.YouTube)
    },
    {
      id: "biliintl",
      rule: ["GEOSITE,biliintl,哔哩哔哩东南亚"],
      name: "哔哩哔哩东南亚",
      url: "https://www.bilibili.tv/",
      icon: ICON_VAL(ICONS.Bilibili3),
      proxiesOrder: ["默认节点", "直连"]
    },
    {
      id: "bahamut",
      rule: ["GEOSITE,bahamut,巴哈姆特"],
      name: "巴哈姆特",
      url: "https://ani.gamer.com.tw/ajax/getdeviceid.php",
      icon: ICON_VAL(ICONS.Bahamut),
      proxiesOrder: ["默认节点", "直连"]
    },
    {
      id: "disney",
      rule: ["GEOSITE,disney,Disney+"],
      name: "Disney+",
      url: "https://disney.api.edge.bamgrid.com/devices",
      icon: ICON_VAL(ICONS.DisneyPlus)
    },
    {
      id: "netflix",
      rule: ["GEOSITE,netflix,NETFLIX"],
      name: "NETFLIX",
      url: "https://api.fast.com/netflix/speedtest/v2?https=true",
      icon: ICON_VAL(ICONS.Netflix)
    },
    {
      id: "tiktok",
      rule: ["GEOSITE,tiktok,Tiktok"],
      name: "Tiktok",
      url: "https://www.tiktok.com/",
      icon: ICON_VAL(ICONS.TikTok)
    },
    {
      id: "spotify",
      rule: ["GEOSITE,spotify,Spotify"],
      name: "Spotify",
      url: "http://spclient.wg.spotify.com/signup/public/v1/account",
      icon: ICON_VAL(ICONS.Spotify)
    },
    {
      id: "pixiv",
      rule: ["GEOSITE,pixiv,Pixiv"],
      name: "Pixiv",
      url: "https://www.pixiv.net/favicon.ico",
      icon: ICON_VAL(ICONS.Pixiv)
    },
    {
      id: "hbo",
      rule: ["GEOSITE,hbo,HBO"],
      name: "HBO",
      url: "https://www.hbo.com/favicon.ico",
      icon: ICON_VAL(ICONS.HBO)
    },
    {
      id: "tvb",
      rule: ["GEOSITE,tvb,TVB"],
      name: "TVB",
      url: "https://www.tvb.com/logo_b.svg",
      icon: ICON_VAL(ICONS.TVB)
    },
    {
      id: "primevideo",
      rule: ["GEOSITE,primevideo,Prime Video"],
      name: "Prime Video",
      url: "https://m.media-amazon.com/images/G/01/digital/video/web/logo-min-remaster.png",
      icon: ICON_VAL(ICONS.PrimeVideo)
    },
    {
      id: "hulu",
      rule: ["GEOSITE,hulu,Hulu"],
      name: "Hulu",
      url: "https://auth.hulu.com/v4/web/password/authenticate",
      icon: ICON_VAL(ICONS.Hulu)
    },
    {
      id: "telegram",
      rule: ["GEOIP,telegram,Telegram"],
      name: "Telegram",
      url: "http://www.telegram.org/img/website_icon.svg",
      icon: ICON_VAL(ICONS.Telegram)
    },
    {
      id: "whatsapp",
      rule: ["GEOSITE,whatsapp,WhatsApp"],
      name: "WhatsApp",
    ...
    {
      id: "line",
      rule: ["GEOSITE,line,Line"],
      name: "Line",
      url: "https://line.me/page-data/app-data.json",
      icon: ICON_VAL(ICONS.Line)
    },
    {
      id: "games",
      rule: ["GEOSITE,category-games@cn,国内网站", "GEOSITE,category-games,游戏专用"],
      name: "游戏专用",
      icon: ICON_VAL(ICONS.Game)
    },
    {
      id: "tracker",
      rule: ["GEOSITE,tracker,跟踪分析"],
      name: "跟踪分析",
      icon: ICON_VAL(ICONS.Reject),
      proxies: ["REJECT", "直连", "默认节点"]
    },
    {
      id: "ads",
      rule: ["GEOSITE,category-ads-all,广告过滤", "RULE-SET,adblockmihomo,广告过滤"],
      name: "广告过滤",
      icon: ICON_VAL(ICONS.Advertising),
      proxies: ["REJECT", "直连", "默认节点"],
      ruleProvider: {
        name: "adblockmihomo", 
        url: URLS.rulesets.adblock_mihomo_mrs(), 
        format: "mrs", 
        behavior: "domain"
      }
    },
    {
      ...
      icon: ICON_VAL(ICONS.Apple2)
    },
    {
      id: "google",
      rule: ["GEOSITE,google,谷歌服务"],
      name: "谷歌服务",
      url: "http://www.google.com/generate_204",
      icon: ICON_VAL(ICONS.GoogleSearch)
    },
    {
      id: "microsoft",
      rule: ["GEOSITE,microsoft@cn,国内网站", "GEOSITE,microsoft,微软服务"],
      name: "微软服务",
      url: "http://www.msftconnecttest.com/connecttest.txt",
      icon: ICON_VAL(ICONS.Microsoft)
    },
    {
      id: "github",
      rule: ["GEOSITE,github,Github"],
      name: "Github",
      url: "https://github.com/robots.txt",
      icon: ICON_VAL(ICONS.GitHub)
    },
    {
      id: "japan",
      rule: ["RULE-SET,category-bank-jp,日本网站", "GEOIP,jp,日本网站,no-resolve"],
      name: "日本网站",
      url: "https://r.r10s.jp/com/img/home/logo/touch.png",
      icon: ICON_VAL(ICONS.JP),
      ruleProvider: {
        name: "category-bank-jp", 
        url: URLS.rulesets.category_bank_jp_mrs(), 
        format: "mrs", 
        behavior: "domain"
      }
    }
  ...
  common: {
    ...
    defaultProxyGroups: [
      {
        name: "下载软件",
        icon: ICON_VAL(ICONS.Download),
        proxies: ["直连", "REJECT", "默认节点", "国内网站"]
      },
      {
        name: "其他外网",
        icon: ICON_VAL(ICONS.StreamingNotCN),
        proxies: ["默认节点", "国内网站"]
      },
      {
        name: "国内网站",
        url: "http://wifi.vivo.com.cn/generate_204",
        icon: ICON_VAL(ICONS.StreamingCN),
        proxies: ["直连", "默认节点"]
      }
    ],
    postRules: [
...
class NodeManager extends EventEmitter {
  ...
  _cooldownTime(id) {
    ...
    return Utils.clamp(
      CONSTANTS.BASE_SWITCH_COOLDOWN * (1 + (s / 100) * 0.9),
  ...
  _recordSwitchEvent(oldId, newId, targetGeo) {
    Logger.debug("SwitchEvent", {
      timestamp: Utils.now(),
      oldNodeId: oldId,
      newNodeId: newId,
      targetGeo: targetGeo ? {
        country: targetGeo.country,
        region: targetGeo.regionName || targetGeo.region
      } : null,
      reason: oldId ? "质量过低" : "初始选择"
    });
  }
  ...
  _updateNodeHistory(id, score) {
    ...
    this.nodeHistory.set(
      id,
      h.length > CONSTANTS.MAX_HISTORY_RECORDS ? h.slice(-CONSTANTS.MAX_HISTORY_RECORDS) : h
    );
  }
  
  updateNodeQuality(id, delta) {
    const ns = Utils.clamp(
      (this.nodeQuality.get(id) || 0) + Utils.clamp(Number(delta) || 0, -20, 20),
      0, 100
    );
  ...
  async switchToNode(id, targetGeo) {
    ...
    const central = CentralManager.getInstance?.();
    const node = central?.state?.config?.proxies?.find(n => n?.id === id);
    
    if (!node) {
      Logger.warn(`尝试切换到不存在的节点: ${id}`);
      return null;
    }
    
    const oldId = this.currentNode;
    this.currentNode = id;
    this.switchCooldown.set(id, Utils.now() + this._cooldownTime(id));
    this._recordSwitchEvent(oldId, id, targetGeo);
    
    const st = central.state.nodes?.get(id);
    const region = st?.geoInfo?.region || st?.geoInfo?.regionName || "未知区域";
  ...
  _best(nodes) {
    ...
    const scoreFor = (node) => {
      ...
      const successRate = Utils.clamp(
        (this.nodeSuccess.get(node.id)?.rate || 0) * 100,
        0, 100
      );
      
      const qw = CONSTANTS.QUALITY_WEIGHT,
            mw = CONSTANTS.METRIC_WEIGHT,
            sw = CONSTANTS.SUCCESS_WEIGHT,
            tw = qw + mw + sw || 1;
      ...
      return Utils.clamp(
        ...
        (metricScore * (mw / tw)) + 
        (successRate * (sw / tw)) + 
        (avail < CONSTANTS.AVAILABILITY_MIN_RATE ? CONSTANTS.BIAS_AVAIL_PENALTY_BAD : 0),
        0, 100
      );
    };
    
    return nodes.reduce(
      (best, n) => (scoreFor(n) > scoreFor(best) ? n : best),
      nodes[0]
    );
  }
  ...
  async getBestNode(nodes, targetGeo) {
    ...
    if (st && regionName) {
      const regional = pool.filter(n => {
        const g = st.get(n.id)?.geoInfo;
        return g && (g.regionName === regionName || g.region === regionName);
      });
  ...
  async switchToBestNode(nodes, targetGeo) {
    ...
    const best = await this.getBestNode(nodes, targetGeo);
    
    if (!best) return null;
    
    const oldId = this.currentNode;
    this.currentNode = best.id;
    this.switchCooldown.set(best.id, Utils.now() + this._cooldownTime(best.id));
    this._recordSwitchEvent(oldId, best.id, targetGeo);
    
    const st = CentralManager.getInstance().state.nodes.get(best.id);
    const region = st?.geoInfo?.region || st?.geoInfo?.regionName || "未知区域";
...
class RegionAutoManager {
  ...
  _hasRegion(regions, name) {
    return Array.isArray(regions) && regions.some(r => r?.name === name);
  }
    ...
    proxies.forEach(p => {
      ...
      for (const e of this.knownRegexMap) 
        if (e.regex.test(name)) 
          found.set(e.name, { name: e.name, regex: e.regex, icon: e.icon });
      ...
      if (hints?.length) {
        ...
        hints.forEach(h => {
          ...
          if (wl[k]) {
            const cnName = wl[k].replace(/[A-Z]{2}/, '').replace(/[^\u4e00-\u9fa5]/g, '');
            const regex = new RegExp(`${k}|${cnName}`, 'i');
  ...
  buildRegionGroups(config, regions) {
    ...
    return {
      regionProxyGroups,
      otherProxyNames: Array.from(new Set(otherNames))
    };
  }
}
...
class CentralManager extends EventEmitter {
  ...
  constructor() {
    ...
    Promise.resolve()
      .then(() => this.initialize().catch(err => Logger.error("CentralManager 初始化失败:", err?.stack || err)));
  }
  ...
  static scoreComponents(m = {}) {
    ...
    const throughputScore = Utils.clamp(
      Math.round(Math.log10(1 + bps) * 2),
      0,
      CONSTANTS.THROUGHPUT_SCORE_MAX
    );
    ...
    return {
      ...
      lossScore,
      throughputScore,
      metricScore: Utils.clamp(
        Math.round(latencyScore + jitterScore + lossScore + throughputScore),
        0, 100
      )
    };
  }
  ...
  async _safeFetch(url, options = {}, timeout = CONSTANTS.GEO_INFO_TIMEOUT) {
    ...
    if (!_fetch)
      throw new Error("fetch 不可用于当前运行环境，且未找到可回退的实现");
    ...
    if (_AbortController && timeout > 0) {
      ...
      try {
        const resp = await _fetch(url, { ...opts, signal: c.signal });
        clearTimeout(tid);
        return resp;
      } catch (err) {
        clearTimeout(tid);
    ...
    if (timeout > 0)
      return Promise.race([
        ...
        new Promise((_, rej) => 
          setTimeout(
            () => rej(new Error(`请求超时 (${timeout}ms): ${url}`)),
            timeout
          )
        )
      ]);
  ...
  async initialize() {
    try {
      ...
      Logger.warn("初始化阶段 GH 镜像预选失败:", e?.message || e);
    }
    
    await this.loadAIDBFromFile().catch(err => 
      Logger.warn("加载AI数据失败，使用默认值:", err?.message || err)
    );
    ...
    if (!this._listenersRegistered) {
      try {
        ...
        Logger.warn("设置事件监听器失败:", e?.message || e);
      }
    }
    
    this.on("requestDetected", (ip) => 
      this.handleRequestWithGeoRouting(ip).catch(err => 
        Logger.warn("地理路由处理失败:", err?.message || err)
      )
    );
    
    this.preheatNodes().catch(err => 
      Logger.warn("节点预热失败:", err?.message || err)
    );
    ...
    try {
      if (PLATFORM.isNode && process.on) {
        const cleanup = () => 
          this.destroy().catch(err => 
            Logger.error("清理资源失败:", err?.message || err)
          );
      ...
      Logger.warn("注册清理函数失败:", e?.message || e);
    }
  ...
  async destroy() {
    ...
    try {
      ...
      Logger.warn("清理事件监听器失败:", e?.message || e);
    }
    
    try {
      await this.saveAIDBToFile();
    } catch (e) {
      Logger.warn("保存AI数据失败:", e?.message || e);
    }
    ...
    try {
      ...
      Logger.warn("清理缓存失败:", e?.message || e);
    }
  ...
  setupEventListeners() {
    this.eventListeners = {
      ...
      performanceThresholdBreached: async (nodeId) => 
        this.onPerformanceThresholdBreached(nodeId),
  ...
  cleanupEventListeners() {
    ...
    try {
      if (typeof Config !== "undefined" && Config.off)
        Config.off("configChanged", this.eventListeners.configChanged);
    } catch {}
    
    try {
      if (PLATFORM.isBrowser && window.removeEventListener)
        window.removeEventListener("online", this.eventListeners.networkOnline);
    } catch {}
    
    try {
      if (this.nodeManager?.off)
  ...
  async preheatNodes() {
    ...
    const tasks = testNodes.map(node => 
      () => Utils.retry(() => this.testNodeMultiMetrics(node), 2, 200)
    );
    ...
    results.forEach((res, i) => {
      ...
      if (res?.__error) {
        Logger.error(`节点预热失败: ${node.id}`, res.__error?.message || res.__error);
        return;
      }
      ...
      this.state.updateNodeStatus(node.id, {
        initialMetrics: enriched,
        lastTested: Utils.now()
      });
  ...
  calculateQuality(metrics) {
    return CentralManager.scoreComponents(metrics || {}).metricScore;
  }
  ...
  async evaluateAllNodes() {
    ...
    results.forEach((r, idx) => {
      if (r?.__error) {
        ...
        Logger.warn(`节点评估失败: ${node?.id}`, r.__error?.message || r.__error);
      }
    });
  ...
  async evaluateNodeQuality(node) {
    ...
    const success = !!(
      ...
      !hardFail && 
      latency > 0 && 
      latency < (CONSTANTS.NODE_TEST_TIMEOUT * 2) && 
      !isSim
    );
    ...
    let score = 0;
    try {
      score = Utils.clamp(this.calculateQuality(metrics), 0, 100);
    } catch (e) {
    ...
    let geoInfo = null;
    try {
      const ip = (node.server && typeof node.server === "string") ? 
        node.server.split(":")[0] : null;
      
      if (Utils.isIPv4(ip) && !Utils.isPrivateIP(ip))
        geoInfo = this.isGeoExternalLookupEnabled() ? 
          await this.getGeoInfo(ip) : 
          this._getFallbackGeoInfo();
    } catch (e) {
    ...
    try {
      ...
      if (
        isCurrent && 
        (hardFail || availRate < CONSTANTS.AVAILABILITY_MIN_RATE || score < CONSTANTS.QUALITY_SCORE_THRESHOLD)
      ) {
        const proxies = this.state?.config?.proxies;
  ...
  async handleRequestWithGeoRouting(targetIp) {
    ...
    if (!targetIp || !nodes.length) {
      Logger.warn("无法进行地理路由: 缺少目标IP或代理节点");
      return;
    }
    
    const targetGeo = this.isGeoExternalLookupEnabled() ? 
      await this.getGeoInfo(targetIp) : 
      this._getFallbackGeoInfo();
    
    if (!targetGeo) {
      Logger.warn("无法获取目标IP地理信息，使用默认路由");
      await this.nodeManager.switchToBestNode(nodes);
      return;
    }
  ...
  autoEliminateNodes() {
    const proxies = this.state.config.proxies || [];
    const threshold = Utils.now() - CONSTANTS.NODE_EVALUATION_THRESHOLD;
    ...
    proxies.forEach(node => {
      ...
      if (
        (!st || st.lastEvaluated < threshold) || 
        (st?.score < CONSTANTS.NODE_CLEANUP_THRESHOLD)
      ) {
  ...
  async onRequestOutbound(reqCtx = {}) {
    ...
    const nodes = this.state.config.proxies || [];
    if (!nodes.length) return { mode: "direct" };
    
    const urlStr = typeof reqCtx.url === "string" ? 
      reqCtx.url : 
      (reqCtx.url?.toString?.() || "");
    ...
    try {
      if (urlStr) {
        ...
        protocol = protocol || 
          (u.protocol || "").replace(":", "").toLowerCase();
        port = port || 
          (u.port ? 
            Number(u.port) : 
    ...
    const clientGeo = clientIP ? 
      (this.isGeoExternalLookupEnabled() ? 
        await this.getGeoInfo(clientIP) : 
        this._getFallbackGeoInfo(hostname)
      ) : null;
    ...
    try {
      if (hostname && Utils.isValidDomain(hostname)) {
        ...
        if (targetIP)
          targetGeo = this.isGeoExternalLookupEnabled() ? 
            await this.getGeoInfo(targetIP) : 
            this._getFallbackGeoInfo(hostname);
      }
    } catch {}
    ...
    const isVideo = !!(
      reqCtx.headers?.["Content-Type"]?.includes("video") || 
      CONSTANTS.STREAM_HINT_REGEX.test(urlStr)
    );
    ...
    const enriched = nodes.map(n => {
      const st = this.state.nodes.get(n.id) || {};
      const m = st.metrics || {};
      
      return {
        node: n,
        score: st.score || 0,
        availability: st.availabilityRate || 0,
    ...
    const bias = (c) => 
      c.score + 
      ((c.availability >= CONSTANTS.AVAILABILITY_MIN_RATE) ? 
        CONSTANTS.BIAS_AVAIL_BONUS_OK : 
        CONSTANTS.BIAS_AVAIL_PENALTY_BAD
      ) + 
      (preferHighThroughput ? 
        Math.min(10, Math.round(Math.log10(1 + c.bps) * 2)) : 0
      ) + 
      (preferLowLatency ? 
        Utils.clamp(
          CONSTANTS.BIAS_LATENCY_MAX_BONUS - (c.latency / 30), 
          0, 
          CONSTANTS.BIAS_LATENCY_MAX_BONUS
        ) : 0
      ) - 
      (preferStability ? 
        Math.min(
          CONSTANTS.BIAS_JITTER_MAX_PENALTY, 
          Math.round(c.jitter / 50)
        ) : 0
      );
    
    let candidates = enriched;
    
    const regionPreferred = (
      targetGeo?.country && 
      Array.isArray(Config.regionOptions?.regions)
    ) ? 
      Utils.filterProxiesByRegion(
        nodes, 
        Config.regionOptions.regions.find(r => 
          r && (
            (r.name?.includes(targetGeo.country)) || 
            (r.regex?.test(targetGeo.country))
          )
        )
      ) : null;
    ...
    const ordered = (candidates.length ? candidates : enriched)
      .sort((a, b) => bias(b) - bias(a))
      .map(c => c.node);
    
    const bestNode = await this.nodeManager.getBestNode(
      ordered.length ? ordered : nodes, 
      targetGeo
    );
    ...
    try {
      if (selected?.id) this.lruCache.set(cacheKey, selected.id);
    } catch {}
  ...
  async handleProxyRequest(req, ...args) {
    ...
    try {
      ...
      const current = dispatch.node || 
        (await this.nodeManager.switchToBestNode(this.state.config.proxies, dispatch.targetGeo));
  ...
  async smartDispatchNode(user, nodes, context) {
    ...
    const hostname = context.req?.url ? 
      (typeof context.req.url === "string" ? 
        new URL(context.req.url).hostname : 
        (context.req.url.hostname || "unknown")
      ) : "unknown";
    ...
    let cached = null;
    try {
      cached = this.lruCache?.get(cacheKey);
    } catch (e) {
    ...
    if (cached) {
      try {
        const node = nodes.find(n => n?.id === cached);
        if (node) return node;
      } catch (e) {
    ...
    const contentType = typeof context.req?.headers?.["Content-Type"] === "string" ? 
      context.req.headers["Content-Type"] : "";
    
    const url = context.req?.url ? 
      (typeof context.req.url === "string" ? 
        context.req.url : 
        context.req.url.toString()
      ) : "";
    ...
    if (contentType.includes("video") || (url && /youtube|netflix|stream/i.test(url))) {
      try {
        const candidateIds = Array.from(this.state.nodes.entries())
          .filter(([_, node]) => 
            typeof node?.score === "number" && node.score > CONSTANTS.QUALITY_SCORE_THRESHOLD
          )
          .map(([id]) => id);
        ...
        if (candidates.length) {
          const tests = candidates.slice(0, limit * 2)
            .map(n => 
              () => Utils.retry(
                () => this.testNodeMultiMetrics(n),
                CONSTANTS.MAX_RETRY_ATTEMPTS,
                CONSTANTS.RETRY_DELAY_BASE
              )
            );
    ...
    if (context.targetGeo?.country && Array.isArray(Config.regionOptions?.regions)) {
      try {
        const targetRegion = Config.regionOptions.regions.find(r => 
          r && (
            (r.name?.includes(context.targetGeo.country)) || 
            (r.regex?.test(context.targetGeo.country))
          )
        );
        ...
        if (targetRegion) {
          ...
          if (regionNodes?.length) {
            const candidates = nodes.filter(n => 
              n?.name && regionNodes.includes(n.name)
            );
            ...
            if (candidates.length) {
              ...
              if (bn) {
                try {
                  this.lruCache?.set(cacheKey, bn.id);
                } catch (e) {
  ...
  async getGeoInfo(ip, domain) {
    ...
    if (!this.isGeoExternalLookupEnabled()) {
      const d = this._getFallbackGeoInfo(domain);
      this.geoInfoCache.set(ip, d, CONSTANTS.GEO_FALLBACK_TTL);
      return d;
    }
    ...
    try {
      ...
      if (primary) {
        this.geoInfoCache.set(ip, primary);
        return primary;
      }
      
      const fallback = await this._fetchGeoFromFallbackAPI(ip);
      if (fallback) {
        this.geoInfoCache.set(ip, fallback);
        return fallback;
      }
  ...
  _getFallbackGeoInfo(domain) {
    if (domain && typeof domain === "string" && Utils.isValidDomain(domain)) {
      ...
      if (map[tld])
        return { country: map[tld], region: "Unknown" };
    }
  ...
  async resolveDomainToIP(domain) {
    if (!Utils.isValidDomain(domain)) {
      Logger.error(`无效的域名参数或格式: ${domain}`);
      return null;
    }
    ...
    try {
      const queries = doh.map(u => 
        this._safeFetch(
          `${u}?name=${encodeURIComponent(domain)}&type=A`,
          {
            headers: {
      ...
      if (ans?.data) {
        this.lruCache.set(cacheKey, ans.data, 600000);
        return ans.data;
      }
  ...
  async proxyRequestWithNode(node, ...args) {
    ...
    if (!node.id || !(node.server || node.proxyUrl))
      throw new InvalidRequestError(
        `代理请求失败: 节点缺少必要属性 (id: ${node?.id}, server: ${node?.server}, proxyUrl: ${node?.proxyUrl})`
      );
    
    try {
      const start = Utils.now();
      const fetchOptions = (args && args.length && typeof args[0] === "object") ? 
        args[0] : {};
      
      const response = await this._safeFetch(
        node.proxyUrl || (node.server ? `http://${node.server}` : ""),
        fetchOptions,
        CONSTANTS.NODE_TEST_TIMEOUT
      );
  ...
  recordRequestMetrics(node, result, req) {
    ...
    const metrics = {
      ...
      bps: this.throughputEstimator.bpsFromBytesLatency({
        bytes: result.bytes || 0,
  ...
  aiScoreNode(node, metrics) {
    ...
    if (recents.length < CONSTANTS.MIN_SAMPLE_SIZE)
    ...
    if (CONSTANTS.ENABLE_SCORE_DEBUGGING && Math.abs(adj) > 3)
      Logger.debug(`Node ${node.id} score components:`, {
        risk: p.risk,
        latency: f.currentLatency,
        loss: f.currentLoss,
        adjustment: adj
      });
  ...
  extractNodeFeatures(node, currentMetrics, recentMetrics, history) {
    ...
    const avgLatency = lat.length ? 
      lat.reduce((a, b) => a + b, 0) / lat.length : 0;
    
    const latencyStd = Utils.calculateStdDev(lat);
    const latencyCV = (latencyStd / (avgLatency || 1)) || 0;
    
    return {
      currentLatency: Number.isFinite(currentMetrics.latency) ? 
        currentMetrics.latency : 0,
      
      currentLoss: Number.isFinite(currentMetrics.loss) ? 
        currentMetrics.loss : 0,
      
      currentJitter: Number.isFinite(currentMetrics.jitter) ? 
        currentMetrics.jitter : 0,
      
      currentBps: Number.isFinite(currentMetrics.bps) ? 
        currentMetrics.bps : 0,
      ...
      latencyCV,
      avgLoss: loss.length ? 
        loss.reduce((a, b) => a + b, 0) / loss.length : 0,
      
      weightedLoss,
      avgJitter: jit.length ? 
        jit.reduce((a, b) => a + b, 0) / jit.length : 0,
      
      avgBps: bps.length ? 
        bps.reduce((a, b) => a + b, 0) / bps.length : 0,
      
      successRate,
      latencyTrend: Utils.calculateTrend(lat),
      lossTrend: Utils.calculateTrend(loss),
      successTrend: Utils.calculateTrend(suc),
      qualityTrend: history?.length >= 2 ? 
        history[history.length - 1].score - history[history.length - 2].score : 0,
      
      recentQuality: history?.length ? 
  ...
  predictNodeFuturePerformance(f) {
    const w = this.getDynamicRiskWeights(f);
    let risk = 0;
    ...
    return {
      risk,
      expectedLatency: f.weightedLatency + f.latencyTrend * 5,
      expectedStability: 1 - risk,
      stabilityScore,
      confidence: Math.min(1, f.sampleSize / CONSTANTS.FEATURE_WINDOW_SIZE)
    };
  }
  ...
  getDynamicRiskWeights(f) {
    ...
    return Object.keys(base).reduce((acc, k) => 
      (acc[k] = base[k] / sum, acc), {}
    );
  }
  
  calculateScoreAdjustment(p, success) {
    if (!success) return -10;
    if (p.risk < 0.3) return 5;
    if (p.risk < 0.5) return 2;
    if (p.risk > 0.7) return -3;
    return 0;
  }
  ...
  processConfiguration(config) {
    ...
    try {
      ...
      throw new ConfigurationError(
        `配置对象无法深拷贝: ${e?.message || "unknown error"}`
      );
    }
    ...
    const providerCount = (
      typeof safe?.["proxy-providers"] === "object" && 
      safe["proxy-providers"] !== null
    ) ? 
      Object.keys(safe["proxy-providers"]).length : 0;
    ...
    try {
      if (Config?.system && typeof Config.system === "object")
        Object.assign(safe, Config.system);
    ...
    try {
      ...
      Config.regionOptions.regions = this.regionAutoManager.mergeNewRegions(
        Config.regionOptions?.regions || [], 
        discovered
      );
    } catch (e) {
      Logger.warn("自动发现新区域失败（不影响原逻辑）:", e.message);
    }
    
    const { regionProxyGroups, otherProxyNames } = this.regionAutoManager.buildRegionGroups(
      safe, 
      Config.regionOptions.regions || []
    );
    ...
    try {
      ...
      if (!safe.proxies.some(p => p?.name === "直连"))
        safe.proxies.push({
          name: "直连",
          type: "direct"
        });
    ...
    try {
      if (regionProxyGroups.length)
        safe["proxy-groups"] = (safe["proxy-groups"] || []).concat(regionProxyGroups);
    } catch (e) {
    ...
    try {
      if (Config.common?.postRules?.length)
    ...
    try {
      if (ruleProviders.size)
        safe["rule-providers"] = Object.fromEntries(ruleProviders);
    } catch (e) {
  ...
  selfTest() {
    try {
      ...
      if (!groups.includes("HK香港"))
        throw new Error("未生成香港分组");
      
      if (!groups.includes("US美国"))
        throw new Error("未生成美国分组");
      
      if (!groups.includes("ES西班牙"))
        throw new Error("未自动识别ES西班牙分组");
      
      if (!groups.includes("TR土耳其"))
        throw new Error("未识别土耳其分组");
      
      Logger.info("自检通过：自动地区分组生成正常");
    } catch (e) {
      Logger.error("自检失败:", e.message);
    }
  }
}
...
class MetricsManager {
  ...
  append(id, m) {
    ...
    const arr = this.state.metrics.get(id) || [];
    arr.push(m);
    this.state.metrics.set(
      id,
      arr.length > CONSTANTS.FEATURE_WINDOW_SIZE ? 
        arr.slice(-CONSTANTS.FEATURE_WINDOW_SIZE) : 
        arr
    );
  }
}
...
class AvailabilityTracker {
  constructor(state, nodeManager) {
    ...
    if (!this.trackers.get(id))
      this.trackers.set(id, new SuccessRateTracker());
  }
  
  record(id, success, opts = {}) {
    this.ensure(id);
    const t = this.trackers.get(id);
    t.record(success, opts);
    this.state.updateNodeStatus(id, { availabilityRate: t.rate });
  }
  
  rate(id) {
    return this.trackers.get(id)?.rate || 0;
  }
  
  hardFailStreak(id) {
    return this.trackers.get(id)?.hardFailStreak || 0;
  }
}
    ...
    try {
      if (response?.body?.getReader) {
        ...
        while (true) {
          ...
          const v = chunk?.value;
          if (v) {
            const len = v.byteLength || v.length || 0;
            bytes += len;
  ...
  return new Promise((resolve) => {
    try {
      ...
      try {
        ...
        else if (PLATFORM.isBrowser && window.localStorage)
      ...
      if (storage) {
        try {
          raw = typeof storage.getItem === "function" ? 
            (storage.getItem("ai_node_data") || "") : 
            (typeof storage.read === "function" ? 
              (storage.read("ai_node_data") || "") : 
              ""
            );
        } catch (e) {
      ...
      if (raw && typeof raw === "string" && raw.trim()) {
        try {
          ...
          if (typeof data === "object" && data !== null && !Array.isArray(data)) {
            ...
            Object.entries(data).forEach(([id, stats]) => {
              if (id && typeof id === "string" && stats && typeof stats === "object") {
                try {
                  this.state.metrics.set(id, Array.isArray(stats) ? stats : [stats]);
                  loaded++;
                } catch (e) {
          ...
          try {
            ...
            else if (PLATFORM.isBrowser && window.localStorage?.removeItem)
              window.localStorage.removeItem("ai_node_data");
          } catch (delErr) {
            Logger.warn("删除损坏数据失败:", delErr.message);
          }
        }
      }
    } catch (e) {
      Logger.error("AI数据加载失败:", e?.stack || e);
    } finally {
      resolve();
    }
  });
...
CentralManager.prototype.saveAIDBToFile = function () {
  try {
    ...
    try {
      ...
      Logger.error("AI数据保存到存储失败:", e?.message || e);
    }
  } catch (e) {
    Logger.error("AI数据保存失败:", e?.stack || e);
  }
};
...
CentralManager.prototype.testNodeMultiMetrics = async function (node) {
  ...
  const probe = async () => {
    const probeUrl = node.proxyUrl || node.probeUrl || 
    ...
    if (!probeUrl)
      throw new Error("无探测URL，使用模拟测试");
    
    const start = Utils.now();
    let response;
    
    try {
      response = await this._safeFetch(probeUrl, { method: "GET" }, timeout);
    } catch {
      return {
        latency: timeout,
        loss: 1,
    ...
    const latency = Utils.now() - start;
    const measure = await this.throughputEstimator.measureResponse(response);
  ...
  try {
    ...
    return new Promise(resolve => {
      setTimeout(() => {
        ...
        try {
          this.lruCache.set(cacheKey, simulated, 60000);
        } catch {}
...
function main(config) {
  ...
  // 1. 执行原有的自动化配置逻辑（生成地区分组、节点清洗等）
  const finalConfig = centralManager.processConfiguration(config);

  // =================自定义逻辑开始=================
  
  // --- 定义关键字列表 ---
  const kwsDomestic = ["c0.jdbstatic.com", "miaogongzi", "jfg", "jifangge", "jfgso", "deepseek"];
  const kwsAdult = ["jav", "missav", "rou", "18comic", "zuise", "jmcomic", "manwa"];

  // --- 辅助函数：查找符合名称的节点 ---
  const findProxies = (regex) => {
    return (finalConfig.proxies || [])
      .filter(p => p.name && regex.test(p.name))
      .map(p => p.name);
  };

  // --- 1. 处理国内/ISP 专线分组 (济南联通/徐州移动) ---
  const groupNameISP = "ISP专线-国内优选";
  // 查找是否有对应的节点
  const specificNodes = findProxies(/济南联通|徐州移动|联通|移动/);
  
  // 构建分组节点列表：优先特定节点 -> 其次直连 -> 再次国内网站
  // 如果你的订阅里没有这俩节点，会自动回退到直连，保证不断网
  const ispGroupProxies = specificNodes.length > 0 
    ? [...specificNodes, "直连"] 
    : ["直连", "国内网站"];
  ...
  const groupISP = {
    ...
    proxies: ispGroupProxies,
    icon: ICON_VAL(ICONS.StreamingCN), // 使用国内图标
    interval: 300,
    tolerance: 20
  };
  ...
  // 获取脚本自动生成的地区分组名称
  const existingGroups = (finalConfig["proxy-groups"] || []).map(g => g.name);
  // 过滤出存在的地区分组
  const targetRegions = ["SG新加坡", "HK香港"].filter(name => existingGroups.includes(name));
  // 兜底：如果没生成这俩地区组，就用默认节点
  const adultGroupProxies = targetRegions.length > 0 
    ? targetRegions 
    : ["默认节点", "其他外网"];
  ...
  const groupAdult = {
    ...Config.common.proxyGroup,
    name: groupNameAdult,
    type: "url-test", // 在新加坡和香港之间自动选最快的
    proxies: adultGroupProxies,
    icon: "https://raw.githubusercontent.com/Koolson/Qure/master/IconSet/Color/Pornhub.png", // 给个图标方便识别
    interval: 300
  };

  // --- 3. 注入分组到配置 ---
  // 将新分组插入到 proxy-groups 的最前面，方便手动切换查看
  ...
  // 生成国内ISP规则
  kwsDomestic.forEach(k => {
    // 优先匹配 DOMAIN-KEYWORD
    newRules.push(`DOMAIN-KEYWORD,${k},${groupNameISP}`);
  });

  // 生成特殊媒体规则
  kwsAdult.forEach(k => {
    newRules.push(`DOMAIN-KEYWORD,${k},${groupNameAdult}`);
  });

  // 将新规则插入到现有规则的最顶部 (Priority High)
  if (Array.isArray(finalConfig.rules)) {
    finalConfig.rules = [...newRules, ...finalConfig.rules];
  } else {
    finalConfig.rules = newRules;
  }

  Logger.info(`自定义注入完成: 国内ISP规则 ${kwsDomestic.length} 条, 特殊媒体规则 ${kwsAdult.length} 条`);
